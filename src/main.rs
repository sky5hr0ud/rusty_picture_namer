...
fn main() {
    println!("Hello, world!");
    let folder_path = "C:\\Users\\distu\\Downloads\\New folder\\Random 2021\\";
    let filetypes_path = "C:\\Users\\distu\\Documents\\Scripts\\Git\\picture_namer\\_list_of_filetypes.txt";
    let result = match directory_walker(folder_path, filetypes_path) {
        Ok(result) => result,
        Err(_) => false
    };
}

fn directory_walker(folder_path: &str, filetypes_path: &str) -> Result<bool, Error> {
    ...
    for directory in directories {
        file_namer(directory.path(), filetypes_path)?;
    }
    return Ok(true)
}

fn file_namer(folder_path: &std::path::Path, filetypes_path: &str) -> Result<bool, Error> {
    ...
    let file_types = get_filetypes(filetypes_path)?;
